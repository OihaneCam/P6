...
public class DataSetMunicipios{
	...
	public ArrayList<String> getListaProvincias(){
		...
		for(Municipio m: lMunicipios) {
			if(!provincias.contains(m.getProvincia())) {
				provincias.add(m.getProvincia());
			}
		}
		return provincias;
	}
	...
	public List<Municipio> getMunicipiosEnProvincia(String provinciaSel){
		List<Municipio> municipiosEnProvincia = new ArrayList<>();
		for(Municipio m: lMunicipios) {
			if(m.getProvincia().equals(provinciaSel)) {
				municipiosEnProvincia.add(m);
			}
		}
		return municipiosEnProvincia;
	}
